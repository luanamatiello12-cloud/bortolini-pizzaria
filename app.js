...
async function api(path, options = {}) {
  ...
  if (state.currentUser?.token) {
    authHeaders["X-Session-Token"] = state.currentUser.token;
  }
  if (state.currentUser?.role) {
    authHeaders["X-User-Role"] = state.currentUser.role;
  }
...
async function loadData() {
  ...
  if (anySuccess) {
    ...
    deliveries = [];
    demoUsers = [
      { email: "admin@bortolini.com", cpf: "00000000000", name: "adm", role: "admin", must_change_pin: 1 },
      { email: "entregador@bortolini.com", cpf: "33333333333", name: "Entregador", role: "entregador", must_change_pin: 1 },
      { email: "financeiro@bortolini.com", cpf: "44444444444", name: "Financeiro", role: "financeiro", must_change_pin: 1 },
    ];
...
function renderDelivery() {
  ...
  activeDrivers.forEach((driver) => {
    ...
    mapPoints.push({
      ...
      label: driver.name,
      sublabel: inRoute ? "Em rota" : "Disponível",
      type: inRoute ? "driver" : "idle"
    });
  });
  deliveries.forEach((delivery) => {
    if (delivery.driver_lat != null && delivery.driver_lng != null) {
      mapPoints.push({
        lat: Number(delivery.driver_lat),
        lng: Number(delivery.driver_lng),
        label: delivery.driver_name || "Entregador",
        sublabel: `Pedido #${delivery.id}`,
        type: "driver"
      });
    }
  });

  renderRealMap("delivery-map", mapPoints);
}
...
// Renderiza mapa de verdade em qualquer cidade do mundo. Antes era um
// "mapa" CSS com posicionamento por fórmula que só fazia sentido para SP.
const _mapInstances = {};

function renderRealMap(containerId, points) {
  if (typeof L === "undefined") {
    // Leaflet ainda não carregou: tenta novamente em 200ms (defer no <script>)
    setTimeout(() => renderRealMap(containerId, points), 200);
    return;
  }
  const el = byId(containerId);
  if (!el) return;
  // O elemento precisa ter altura definida para Leaflet calcular tiles
  if (!el.style.height && el.clientHeight < 80) el.style.height = "260px";

  let map = _mapInstances[containerId];
  if (!map) {
    el.innerHTML = "";  // limpa pins CSS antigos se houver
    ...
    _mapInstances[containerId + "_markers"] = [];
  }

  // Limpa marcadores antigos
  (_mapInstances[containerId + "_markers"] || []).forEach(m => map.removeLayer(m));
  _mapInstances[containerId + "_markers"] = [];

  const valid = (points || []).filter(p => !isNaN(Number(p.lat)) && !isNaN(Number(p.lng)));
  if (!valid.length) {
    // Default: centro do Brasil para não ficar no oceano (-15, -55)
    map.setView([-15.78, -55.92], 4);
    setTimeout(() => map.invalidateSize(), 100);
    return;
  }

  valid.forEach(p => {
    const color = p.type === "destination" ? "#d62828" : p.type === "idle" ? "#888" : "#22a06b";
    const icon = L.divIcon({
      className: "leaflet-bortolini-pin",
      html: `<div style="background:${color};color:#fff;padding:4px 8px;border-radius:14px;font-size:.78rem;font-weight:600;white-space:nowrap;box-shadow:0 2px 6px rgba(0,0,0,.3)">${p.label || "•"}</div>`,
      iconSize: [80, 24],
      iconAnchor: [40, 12]
    });
    const marker = L.marker([p.lat, p.lng], { icon }).addTo(map);
    if (p.sublabel) marker.bindPopup(`<strong>${p.label || ""}</strong><br>${p.sublabel}`);
    _mapInstances[containerId + "_markers"].push(marker);
  });
...
function formatLocation(delivery) {
  if (delivery.driver_lat == null || delivery.driver_lng == null) return "Localização ainda não compartilhada";
  const lat = Number(delivery.driver_lat).toFixed(4);
  const lng = Number(delivery.driver_lng).toFixed(4);
  return `Localização: ${lat}, ${lng} · ${delivery.last_location_at ? "atualizado " + new Date(delivery.last_location_at).toLocaleTimeString("pt-BR") : "agora"}`;
}

function simulateDriverMovement() {
  // DESATIVADA: anteriormente esta função sobrescrevia o GPS real do entregador
  // a cada 7s no frontend, fazendo o mapa "andar" mesmo sem GPS ativo. Removida
  // para que apenas dados reais (vindos do navigator.geolocation do entregador)
  // apareçam no mapa.
}
...
function renderSettings() {
  ...
  byId("settings-menu-link").value = baseUrl + "/cardapio";
...
function renderTeamUsers() {
  ...
  box.innerHTML = demoUsers.length
    ? demoUsers
        .map(
          (user) => {
            const driverLink = user.role === "entregador" ? `<a href="/entregador/${user.id}" target="_blank">🔗 /entregador/${user.id}</a>` : "";
            return `
            <article class="ingredient-card">
              <div>
                <small>${roleLabels[user.role] || user.role}</small>
                <strong>${user.name}</strong>
                <p>CPF: ${formatCpf(user.cpf)}${Number(user.must_change_pin) ? " · primeiro acesso pendente" : ""}</p>
                ${driverLink}
              </div>
...
async function checkoutCart() {
  ...
  try {
    ...
    const trackLink = `${window.location.origin}/pedido/${created.id}`;
...
async function createDriver() {
  ...
  if (!payload.cpf || payload.cpf.replace(/\D/g, "").length < 4) { showToast("CPF com pelo menos 4 dígitos é obrigatório."); return; }
  try {
    const created = state.apiOnline
...
async function resetUserPin(userId) {
  ...
  try {
    ...
    renderTeamUsers();
    showToast("PIN resetado para os 4 primeiros números do CPF.");
  } catch (error) {
...
async function recoverAdminAccess() {
  const cpf = byId("recover-admin-cpf").value.trim();
  ...
  if (!cpf || !masterKey || !/^\d{4,6}$/.test(newPin)) {
    byId("recover-admin-message").textContent = "Preencha CPF, chave mestra e novo PIN com 4 a 6 números.";
    return;
  }
  try {
    await api("/api/admin/recover", {
      method: "POST",
      body: JSON.stringify({ cpf, master_key: masterKey, new_pin: newPin }),
    });
    byId("recover-admin-dialog").close();
    byId("login-email").value = cpf;
...
function renderDemoUsers() {
  const fallbackProfiles = [
    { cpf: "00000000000", name: "adm", role: "admin" },
    { cpf: "44444444444", name: "Financeiro", role: "financeiro" },
  ];
  const profiles = demoUsers.length ? demoUsers : fallbackProfiles;
  byId("demo-users").innerHTML = profiles
    .map(
      (user) => `
        <button class="profile-button" data-demo-login="${user.cpf || user.email}" data-demo-pin="${String(user.cpf || "1234").replace(/\D/g, "").slice(0, 4) || "1234"}">
          <strong>${roleLabels[user.role] || user.name}</strong>
          <small>CPF ${formatCpf(user.cpf)} · PIN inicial ${String(user.cpf || "1234").replace(/\D/g, "").slice(0, 4) || "1234"}</small>
        </button>
      `,
    )
    .join("");
...
function showApp() {
  ...
  renderAll();
  // Se veio de /admin, fica no painel; senão vai pra primeira view permitida
  const intended = window._redirectAfterLogin;
  window._redirectAfterLogin = null;
  switchView(intended && can(intended) ? intended : firstAllowedView());
  // Limpa a URL amigável para não ficar mostrando /admin na barra
...
let _driverToken = null;  // token de sessão obtido após login do entregador
let _driverRole = "entregador";

function driverAuthHeaders() {
  const headers = {"Content-Type": "application/json"};
  if (_driverToken) headers["X-Session-Token"] = _driverToken;
  if (_driverRole) headers["X-User-Role"] = _driverRole;
  return headers;
}
...
function initDriverPublicPage() {
  ...
  // Mostrar tela de login do entregador antes do conteúdo
  showDriverLoginGate();
}

function showDriverLoginGate() {
  // Esconder pedidos e mostrar formulário de PIN
  const ordersEl = byId("driver-public-orders");
  if (ordersEl) {
    ordersEl.innerHTML = `
      <div class="driver-login-gate" style="padding:1rem;background:var(--card,#fff);border-radius:12px;max-width:320px;margin:1rem auto">
        <h3 style="margin:0 0 .5rem">Entrar como entregador</h3>
        <p style="font-size:.9rem;color:var(--muted,#666);margin:0 0 1rem">Informe o PIN cadastrado. Caso seja o primeiro acesso, use os 4 primeiros dígitos do seu CPF.</p>
        <input id="driver-login-pin" type="password" inputmode="numeric" pattern="[0-9]*" maxlength="6" placeholder="PIN" style="width:100%;padding:.6rem;font-size:1.1rem;letter-spacing:.3rem;text-align:center;margin-bottom:.5rem">
        <button id="driver-login-btn" class="primary" style="width:100%;padding:.6rem">Entrar</button>
        <p id="driver-login-error" style="color:var(--danger,#c00);font-size:.85rem;margin-top:.5rem;min-height:1.2rem"></p>
      </div>
    `;
    byId("driver-login-btn")?.addEventListener("click", driverLogin);
    byId("driver-login-pin")?.addEventListener("keypress", (e) => {
...
async function driverLogin() {
  ...
  const errEl = byId("driver-login-error");
  if (errEl) errEl.textContent = "";
  if (!pin || pin.length < 4) {
    if (errEl) errEl.textContent = "Informe o PIN (mínimo 4 dígitos).";
    return;
  }
  try {
    // Buscar CPF do usuário (via rota pública mínima) — pode pegar pelo próprio user_id
    // O login exige CPF ou email + PIN. Usamos o id pra buscar via tentativa simples.
    const resp = await fetch("/api/login", {
      method: "POST",
      headers: {"Content-Type": "application/json"},
      body: JSON.stringify({ driver_user_id: _driverUserId, pin })
    });
    if (!resp.ok) {
      const text = await resp.text();
      const match = text.match(/Message: ([^<]+)/);
      if (errEl) errEl.textContent = match ? match[1].trim() : "PIN inválido.";
      return;
    }
    const data = await resp.json();
    _driverToken = data.token;
    _driverRole = data.role || "entregador";
    if (data.must_change_pin) {
      promptDriverChangePin(data.id);
      return;
    }
    loadDriverPublicOrders();
  } catch (e) {
    if (errEl) errEl.textContent = "Erro ao entrar. Tente novamente.";
  }
}

async function promptDriverChangePin(userId) {
  const ordersEl = byId("driver-public-orders");
  if (!ordersEl) return;
  ordersEl.innerHTML = `
    <div style="padding:1rem;background:var(--card,#fff);border-radius:12px;max-width:320px;margin:1rem auto">
      <h3 style="margin:0 0 .5rem">Definir novo PIN</h3>
      <p style="font-size:.9rem;color:var(--muted,#666);margin:0 0 1rem">Por segurança, escolha um PIN pessoal (4 a 6 dígitos).</p>
      <input id="driver-new-pin" type="password" inputmode="numeric" pattern="[0-9]*" maxlength="6" placeholder="Novo PIN" style="width:100%;padding:.6rem;font-size:1.1rem;letter-spacing:.3rem;text-align:center;margin-bottom:.5rem">
      <input id="driver-new-pin-confirm" type="password" inputmode="numeric" pattern="[0-9]*" maxlength="6" placeholder="Confirmar PIN" style="width:100%;padding:.6rem;font-size:1.1rem;letter-spacing:.3rem;text-align:center;margin-bottom:.5rem">
      <button id="driver-change-pin-btn" class="primary" style="width:100%;padding:.6rem">Salvar</button>
      <p id="driver-change-pin-error" style="color:var(--danger,#c00);font-size:.85rem;margin-top:.5rem;min-height:1.2rem"></p>
    </div>
  `;
  byId("driver-change-pin-btn")?.addEventListener("click", async () => {
    const newPin = byId("driver-new-pin").value.trim();
    const confirm = byId("driver-new-pin-confirm").value.trim();
    const err = byId("driver-change-pin-error");
    if (!/^\d{4,6}$/.test(newPin)) { err.textContent = "PIN deve ter 4 a 6 dígitos."; return; }
    if (newPin !== confirm) { err.textContent = "PINs não coincidem."; return; }
    try {
      const resp = await fetch(`/api/users/${userId}/pin`, {
        method: "PATCH",
        headers: driverAuthHeaders(),
        body: JSON.stringify({ new_pin: newPin })
      });
      if (!resp.ok) { err.textContent = "Não foi possível salvar."; return; }
      loadDriverPublicOrders();
    } catch (e) { err.textContent = "Erro ao salvar."; }
  });
}

async function loadDriverPublicOrders() {
  if (!_driverUserId) return;
  if (!_driverToken) { showDriverLoginGate(); return; }
  try {
    const resp = await fetch(`/api/public/driver/${_driverUserId}`, { headers: driverAuthHeaders() });
    if (resp.status === 401 || resp.status === 403) { _driverToken = null; showDriverLoginGate(); return; }
    const data = await resp.json();
    const nameEl = byId("driver-public-name");
    if (nameEl) nameEl.textContent = `Olá, ${data.driver_name}! 🛵`;
...
async function driverMarkDelivered(orderId) {
  try {
    ...
    if (!resp.ok) { showToast("Erro ao atualizar pedido."); return; }
...
function driverShareLocation() {
  ...
  if (!_driverToken) { showToast("Faça login primeiro."); showDriverLoginGate(); return; }
  byId("driver-share-location-btn")?.classList.add("hidden");
  byId("driver-stop-location-btn")?.classList.remove("hidden");
  const statusEl = byId("driver-location-status");
  if (statusEl) statusEl.textContent = "📍 Compartilhando localização...";

  function sendLocation(pos) {
    const { latitude: lat, longitude: lng } = pos.coords;
    if (statusEl) statusEl.textContent = `📍 Localização ativa: ${lat.toFixed(4)}, ${lng.toFixed(4)}`;
  ...
  _driverLocationInterval = navigator.geolocation.watchPosition(
    sendLocation,
    () => { if (statusEl) statusEl.textContent = "⚠️ Erro ao obter GPS."; },
...
function driverStopLocation() {
  ...
  const statusEl = byId("driver-location-status");
  if (statusEl) statusEl.textContent = "📍 Localização pausada.";
}

async function registerDriver() {
  const errEl = byId("driver-reg-error");
  if (errEl) errEl.textContent = "Auto-cadastro desativado. Solicite ao administrador que cadastre você no painel.";
}
...
async function loadOrderTrack(orderId) {
  ...
  try {
    ...
    if (order.status === "Entrega" && order.driver_lat && order.driver_lng) {
      ...
      renderRealMap("track-map-area", [
        { lat: Number(order.driver_lat), lng: Number(order.driver_lng), label: order.driver_name || "Entregador", type: "driver" }
      ]);
    }
  } catch(e) {
...
let _trackPollingInterval = null;
if (trackOrderId && !isNaN(Number(trackOrderId))) {
  document.addEventListener("DOMContentLoaded", () => {
    ...
    // Polling: atualiza a cada 10s o status e a localização do entregador
    if (_trackPollingInterval) clearInterval(_trackPollingInterval);
    _trackPollingInterval = setInterval(() => {
...
// Modo público do cardápio — ativado por /cardapio, /pedido ou #customer
function isCustomerRoute() {
  const p = window.location.pathname;
  return p === "/cardapio" || p === "/pedido" || window.location.hash === "#customer";
}
...
// Atalho /admin → ativa painel admin após login
if (window.location.pathname === "/admin") {
  document.addEventListener("DOMContentLoaded", () => {
    // Guarda intenção: após login bem-sucedido, vai direto pro painel
    window._redirectAfterLogin = "admin";
  });
}
